...
public class WiresTests extends FlowPanel
{
    ...
    public WiresTests(Layer layer, Layer pickerLayer)
    {
        this.layer = layer;
    }
...
//
//        final WiresShape wiresShape1 = wires_manager.createShape(new MultiPath().rect(0, 0, w, h).setStrokeColor("#00CC00")).setX(50).setY(50).setDraggable(true);
//        wiresShape1.getContainer().setUserData("A");
//        wiresShape1.addChild(new Star(5, 15, 40), CENTER);
//
//        final WiresShape wiresShape2 = wires_manager.createShape(new MultiPath().rect(0, 0, 300, 200).setStrokeColor("#0000CC")).setX(50).setY(100).setDraggable(true);
...
//        final WiresShape wiresShape3 = wires_manager.createShape(new MultiPath(svg).setStrokeColor("#0000CC")).setX(50).setY(300).setDraggable(true);
//        wiresShape3.getContainer().setUserData("B");
        final MultiPath path = new MultiPath().rect(100, 100, 200, 300).setDraggable(true);
        final MultiPath indicator = new MultiPath().circle(5).setFillColor("#CC0000");
        layer.add(path);
        layer.add(indicator);
        layer.addNodeMouseMoveHandler(new NodeMouseMoveHandler()
        {
            @Override public void onNodeMouseMove(NodeMouseMoveEvent event)
            {
                int x = event.getX();
                int y = event.getY();
                Console.get().info("m "+new Point2D(x,y).toJSONString());
                Point2D intersection = Geometry.findIntersection(x, y, path);
                if (intersection != null) {
                    Console.get().info("i "+intersection.toJSONString());
                    indicator.setX(intersection.getX()).setY(intersection.getY() - (indicator.getBoundingPoints().getBoundingBox().getHeight()/2));
                    layer.batch();
                }
            }
        });
